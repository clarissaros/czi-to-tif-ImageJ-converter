...
//   based on their position within each pair (every 2 files).
//
// Expected input structure:
//   Files must be sorted as pairs (2 channels per sample):
//     Position 0 → _nuclei   (e.g., DAPI channel)
//     Position 1 → _channel2 (e.g., marker channel)
//
// Usage:
...
// Define the channel suffixes assigned cyclically every 2 files
// Modify these strings to match your experimental staining panel
suffixes = newArray("_nuclei", "_channel2");
...
for (i = 0; i < list.length; i++) {
    ...
    // Determine channel suffix based on file position within the pair
    // Modulo 2 cycles through indices 0 and 1 regardless of total file count
    suffixIndex = i % 2;
